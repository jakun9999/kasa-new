type FavoriteIconProps = {
  className?: string;
  /** Fill / stroke du path. Défaut = comportement card (gris dark, hover rouge light). */
  pathClassName?: string;
};

/**
 * Cœur Figma (`favori_icon.svg`).
 * Repos card : fill Gris dark. Hover du bouton (`group`) : fill rouge light.
 * Contour Gris light dans les deux états card.
 */
export function FavoriteIcon({
  className = "size-4",
  pathClassName = "fill-kasa-gray-dark stroke-kasa-gray-light transition-colors group-hover:fill-kasa-red-light ease-in-out duration-300",
}: FavoriteIconProps) {
  return (
    <svg
      width="16"
