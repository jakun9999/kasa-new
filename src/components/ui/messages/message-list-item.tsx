...
export function MessageListItemSkeleton({
  ...
  return (
    ...
    >
      <div
        className={`h-[45.05px] w-[44.5px] shrink-0 rounded-[5.49px] ${skeletonFill}`}
      />
      <div className="ml-5 flex min-w-0 flex-1 flex-col items-start gap-1">
        <div className={`h-3.5 w-28 max-w-full rounded-kasa-short ${skeletonFill}`} />
        <div
          className={`h-2.5 w-full max-w-47.5 rounded-kasa-short ${skeletonFill}`}
        />
      </div>
      <div className="ml-auto flex h-8.25 w-10.5 shrink-0 flex-col items-end justify-between">
        <div className={`h-2.5 w-10 rounded-kasa-short ${skeletonFill}`} />
        <div className={`size-1.5 rounded-full ${skeletonFill}`} />
      </div>
    </div>
  );
}
 ...
 * En `next dev`, skeleton 2 s pour la démo soutenance ; skeleton photo jusqu’au `onLoad`.
 */
export function MessageListItem({
  ...
  return (
    <button
      ...
      className={`flex h-[61.05px] w-full min-w-0 cursor-pointer items-center border-b border-kasa-gray-light px-2.5 text-left ${
        selected ? "bg-kasa-cream" : "bg-kasa-white"
      } ${className}`.trim()}
    >
      <div className="relative h-[45.05px] w-[44.5px] shrink-0 self-center overflow-hidden rounded-[5.49px] bg-kasa-gray-light">
        {!imageLoaded ? (
          <div className={`absolute inset-0 ${skeletonFill}`} />
        ) : null}
        {correspondentPicture ? (
          <Image
      ...
      <div className="relative ml-auto flex h-8.25 w-10.5 shrink-0 flex-col items-end">
        <time className="text-overline font-normal text-kasa-black">
          {lastMessageAt}
        </time>
        {hasUnread ? (
          <span
